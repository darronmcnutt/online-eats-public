...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Driver is hardcoded to Patrick's UID for testing
        mDriverRef = mDatabase.getReference("drivers/Jw4rj87oKfVzR8ncHNtMgMVWBQn1");

        // Customer is set to the current user's ID
        mCustomerRef = mDatabase.getReference("customers/" + mUser.getUid() + "/location");
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // Set default location for markers (Chicago, IL city center)
        final Marker driverMarker = mMap.addMarker(new MarkerOptions()
                .position(new LatLng(41.881832, -87.623177))
                .title("Driver")
                .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE)));
        final Marker customerMarker = mMap.addMarker(new MarkerOptions()
                .position(new LatLng(41.881832, -87.623177))
                .title("Customer"));

        mDriverRef.addValueEventListener(new ValueEventListener() {
            @Override
            ...
            public void onCancelled(DatabaseError databaseError) {
                System.out.println("Location read from Firebase failed: " + databaseError.getCode());
            }
        });
